    ...
    extends Runner
{
    ...
    private void runBefore( final ScriptTestFile file )
        throws Exception
    {
        this.testInstance.setUp();
        file.runBefore( this.testInstance );
    }

    private void runAfter( final ScriptTestFile file )
        throws Exception
    {
        file.runAfter( this.testInstance );
    ...
    private Engine createEngine( final ScriptTestFile testFile )
    {
    ...
    {
        return this.testInstance.getTestFiles().stream().map( String::trim ).filter( ( str ) -> !Strings.isNullOrEmpty( str ) ).collect(
            Collectors.toList() );
    }
